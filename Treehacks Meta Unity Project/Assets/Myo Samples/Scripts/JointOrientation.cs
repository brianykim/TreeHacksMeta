...
public class JointOrientation : MonoBehaviour
{
    ...
    void Update ()
    {
		...
		Vector3 m1 = Vector3.Cross (myo.transform.forward, antigravity);
		Vector3 roll1 = Vector3.Cross (m1, myo.transform.forward);
		Vector3 pitch1 = Vector3.Cross (roll1, myo.transform.forward);//Quaternion.\
		Quaternion newQuat = new Quaternion();
		Quaternion myo1 = new Quaternion (transform.localRotation.y, transform.localRotation.z, -transform.localRotation.x, -transform.localRotation.w);
		//q.y, q.z, -q.x, -q.w
		//Quaternion newQuat = Quaternion ();
		float pitchCalc = Mathf.Asin (Mathf.Max (-1.0f, Mathf.Min (1.0f, 2.0f * (myo1.w * myo1.y - myo1.z * myo1.x))));
		print ("pitch calc: "+ pitchCalc);
		if(pitchCalc>0)
		{
			pos = true;
		}
		...
		else if(neg&&pitchCalc>0) {
			halfSteps++;
			neg = false;
			print ("step " + halfSteps/2 );
		}
